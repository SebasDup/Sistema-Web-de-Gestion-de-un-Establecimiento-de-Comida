...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(80, 10, 'Nombre del Platillo', 1, 0, 'C', 1);
        $this->Cell(40, 10, 'Precio', 1, 0, 'C', 1);
        $this->Cell(50, 10, 'Veces Pedido', 1, 1, 'C', 1);
    }
...
$query = "SELECT 
    ...
    m.nombre,
    m.precio,
    COALESCE(SUM(ic.cantidad), 0) as veces_pedido
FROM menu m
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $pdf->Cell(40, 10, '$' . number_format($row['precio'], 2), 1, 0, 'C');
        $pdf->Cell(50, 10, utf8_decode($row['veces_pedido']), 1, 1, 'C');
    }
} else {
