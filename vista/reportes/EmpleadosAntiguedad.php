...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $tableWidth = 15 + 45 + 40 + 30 + 35 + 40 + 50; // Aumentado a 50 para antigüedad
        ...
        $this->Cell(35, 10, utf8_decode('Zona Asignada'), 1, 0, 'C', 1);
...
// Corregir la consulta para usar apellidoP y apellidoM en lugar de apellido
$query = "SELECT e.*, u.nombre, u.apellidoP, u.apellidoM, 
...
if ($result) {
    while ($datos_reporte = mysqli_fetch_assoc($result)) {
        ...
        // Concatenar nombre completo con apellidos
        ...
        $pdf->Cell(45, 10, utf8_decode($nombre_completo), 1, 0, 'C', 0);
        $pdf->Cell(40, 10, utf8_decode($datos_reporte['puesto']), 1, 0, 'C', 0);
        $pdf->Cell(30, 10, utf8_decode($datos_reporte['salario']), 1, 0, 'C', 0);
        $pdf->Cell(35, 10, utf8_decode($datos_reporte['zona_asignada']), 1, 0, 'C', 0);
