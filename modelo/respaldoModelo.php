...
class RespaldoModelo {
    ...
    public function realizarRestauracion($filePath) {
        try {
            ...
            // 1. Hacer backup de la tabla bitacora_db
            $bitacora_backup = [];
            $stmt = $this->pdo->query("SELECT * FROM bitacora_db");
            ...
            // 8. Restaurar los registros de bitacora_db
            foreach ($bitacora_backup as $registro) {
                $stmt = $this->pdo->prepare("INSERT INTO bitacora_db (nombre_usuario, tipo_operacion, nombre_archivo, fecha_operacion) VALUES (?, ?, ?, ?)");
                $stmt->execute([
            ...
            // 9. Registrar la operación actual en bitacora
            $stmt = $this->pdo->prepare("INSERT INTO bitacora_db (nombre_usuario, tipo_operacion, nombre_archivo) VALUES (?, ?, ?)");
            $stmt->execute([$_SESSION['usuario'], 'restauracion', $nombre_archivo]);

            // 10. Reactivar verificación de claves foráneas
