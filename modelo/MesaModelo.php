...
class MesaModelo {
    ...
    public function agregarZona($zona, $descripcion) {
        $stmt = $this->pdo->prepare("INSERT INTO zonas (nombre, descripcion) VALUES (?, ?)");
        if ($stmt->execute([$zona, $descripcion])) {
    ...
    public function actualizarZona($id, $nombre, $descripcion){
        $stmt = $this->pdo->prepare("UPDATE zonas SET nombre = ?, descripcion = ? WHERE id = ?");
