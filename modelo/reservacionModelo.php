...
class ReservacionModelo {
    ...
    public function agregarReservacion($cliente_id, $fechaHora, $personas, $estado, $comentarios, $mesa_id) {
        // Verificar que el cliente_id existe y es válido
        if (empty($cliente_id)) {
            $_SESSION['error'] = "Error: Debe seleccionar un cliente válido para la reservación.";
            return false;
        }

        // Verificar que el cliente existe en la base de datos
        $stmtCheck = $this->pdo->prepare("SELECT COUNT(*) FROM usuarios WHERE id = ?");
        $stmtCheck->execute([$cliente_id]);
        if ($stmtCheck->fetchColumn() == 0) {
            $_SESSION['error'] = "Error: El cliente seleccionado no existe en la base de datos.";
            return false;
        }

        // Continuar con la inserción si el cliente es válido
